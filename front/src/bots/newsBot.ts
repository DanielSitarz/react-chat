...
export default class NewsBot extends Bot {
  API_ADDR = 'https://newsapi.org/v1/articles?source='
  API_KEY = '&apiKey=ed34e433fbe2476b958380195cc93017'
  ...
  showHelp() {
    this.sendResponse('!news [category] [article_index or "list"]\n' + this.showAvailableCategories())
  }
  ...
  showArticleForCategory(category) {
    ...
    this.getArticles(category, this.sources[category][0], (data) => {
      this.sendResponse(data[0].title + ' - ' + data[0].description)
    })
  }
  showSpecificArticleForCategory(category, index) {
    ...
    this.getArticles(category, this.sources[category][0], (data) => {
      ...
      this.sendResponse(data[index].title + ' - ' + data[index].description)
    })
  }
  showArticlesForCategory(category) {
    ...
    this.getArticles(category, this.sources[category][0], (data) => {
      ...
      this.sendResponse(msg.join('\n'))
    })
  }

  articleOutOfBounds(maxIndex) {
    this.sendResponse(`Article doesn't exist, max article index is ${maxIndex}.`)
  }
  ...
  categoryDoesntExist(category) {
    this.sendResponse(`Category "${category}" doesn't exist.\n` + this.showAvailableCategories())
  }
